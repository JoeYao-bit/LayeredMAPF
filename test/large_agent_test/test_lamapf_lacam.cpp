...
void startLaCAMTest(const std::vector<AgentType>& agents, const InstanceOrients<2>& instances) {
    ...
    LaCAM::LargeAgentLaCAM<2, AgentType> lacam(instances, agents, dim, is_occupied, &MT);
...
TEST(BlockAgentSubGraph, lacam_test) {
    ...
    startLaCAMTest<BlockAgent_2D>(agents, instances);
...
TEST(CircleAgentSubGraph, lacam_test) {
    ...
    startLaCAMTest<CircleAgent<2> >(agents, instances);
}
