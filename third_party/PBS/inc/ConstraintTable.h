...
namespace PBS_Li {
    ...
    public:
        ...
        void insert2CT(const CBS_Li::MAPFPath &path); // insert a path to the constraint table
        ...
        void insert2CAT(int agent,
                        const std::vector<CBS_Li::MAPFPath *> &paths); // build the conflict avoidance table using a set of paths
        void insert2CAT(const CBS_Li::MAPFPath &path); // insert a path to the collision avoidance table
    ...
    protected:
        ...
        typedef std::vector<std::vector<bool> > CAT;
        CAT cat;
        int cat_max_timestep = 0;
        std::vector<int> cat_goals;
        ...
        std::list<std::pair<int, int> > decodeBarrier(int B1, int B2, int t) const;
