...
namespace PBS_Li {
    ...
    public:
        ...
        typedef boost::heap::pairing_heap<SIPPNode *, boost::heap::compare<SIPPNode::compare_node> >::handle_type open_handle_t;
        typedef boost::heap::pairing_heap<SIPPNode *, boost::heap::compare<SIPPNode::secondary_compare_node> >::handle_type focal_handle_t;
    ...
    public:
        CBS_Li::MAPFPath findOptimalPath(const std::set<int> &higher_agents, const std::vector<CBS_Li::MAPFPath *> &paths, int agent, const CBS_Li::ConstraintTable &initial_constraints) override;

        std::string getName() const override { return "SIPP"; }

        SIPP(const CBS_Li::Instance &instance, int agent) :
    ...
    private:
        ...
        typedef boost::unordered_map<SIPPNode *, std::list<SIPPNode *>, SIPPNode::NodeHasher, SIPPNode::eqnode> hashtable_t;
        hashtable_t allNodes_table;
        std::list<SIPPNode *> useless_nodes;
        // Path findNoCollisionPath(const ConstraintTable& constraint_table);

        void updatePath(const LLNode *goal, std::vector<CBS_Li::PathEntry> &path);
