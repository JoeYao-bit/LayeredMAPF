...
namespace PBS_Li {
    ...
    public:
        ...
        {
            return std::max(get_DH_heuristic(from, to), instance.getManhattanDistance(from, to));
        }

        const CBS_Li::Instance &instance;

        virtual CBS_Li::MAPFPath findOptimalPath(const std::set<int> &higher_agents, const std::vector<CBS_Li::MAPFPath *> &paths, int agent, const CBS_Li::ConstraintTable &initial_constraints) = 0;

        virtual std::string getName() const = 0;

        std::list<int> getNextLocations(int curr) const; // including itself and its neighbors
        std::list<int> getNeighbors(int curr) const { return instance.getNeighbors(curr); }
        ...
        SingleAgentSolver(const CBS_Li::Instance &instance, int agent) :
