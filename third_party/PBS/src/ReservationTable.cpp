#include "PBS/inc/ReservationTable.h"
...
{ 
    ...
    void ReservationTable::insert2SIT(int location, int t_min, int t_max) {
        ...
        for (auto it = sit[location].begin(); it != sit[location].end();) {
            if (t_min >= std::get<1>(*it))
                ...
                (*it) = std::make_tuple(std::get<0>(*it), t_min, std::get<2>(*it));
                ++it;
            } else if (t_min <= std::get<0>(*it) && t_max < std::get<1>(*it)) {
                (*it) = std::make_tuple(t_max, std::get<1>(*it), std::get<2>(*it));
                break;
            } else if (std::get<0>(*it) < t_min && t_max < std::get<1>(*it)) {
                sit[location].insert(it, std::make_tuple(std::get<0>(*it), t_min, std::get<2>(*it)));
                (*it) = std::make_tuple(t_max, std::get<1>(*it), std::get<2>(*it));
                break;
    ...
    void ReservationTable::insertSoftConstraint2SIT(int location, int t_min, int t_max) {
        ...
        for (auto it = sit[location].begin(); it != sit[location].end(); ++it) {
            if (t_min >= std::get<1>(*it) || std::get<2>(*it))
                continue;
            else if (t_max <= std::get<0>(*it))
                break;

            auto i_min = std::get<0>(*it);
            auto i_max = std::get<1>(*it);
            if (i_min < t_min && i_max <= t_max) {
                if (it != sit[location].end() and std::next(it) != sit[location].end() and
                    ...
                    i_max == std::get<0>(*std::next(it)) and
                        std::get<2>(*std::next(it))) // we can merge the current interval with the next one
                {
                    (*it) = std::make_tuple(i_min, t_min, false);
                    ++it;
                    (*it) = std::make_tuple(t_min, std::get<1>(*it), true);
                } else {
                    sit[location].insert(it, std::make_tuple(i_min, t_min, false));
                    (*it) = std::make_tuple(t_min, i_max, true);
                }

            } else if (t_min <= i_min && t_max < i_max) {
                if (it != sit[location].begin() and
                    (location != goal_location || i_min != constraint_table.length_min) and
                    i_min == std::get<1>(*std::prev(it)) and
                        std::get<2>(*std::prev(it))) // we can merge the current interval with the previous one
                {
                    (*std::prev(it)) = std::make_tuple(std::get<0>(*std::prev(it)), t_max, true);
                } else {
                    sit[location].insert(it, std::make_tuple(i_min, t_max, true));
                }
                (*it) = std::make_tuple(t_max, i_max, false);
            } else if (i_min < t_min && t_max < i_max) {
                sit[location].insert(it, std::make_tuple(i_min, t_min, false));
                sit[location].insert(it, std::make_tuple(t_min, t_max, true));
                (*it) = std::make_tuple(t_max, i_max, false);
            } else // constraint_min <= std::get<0>(*it) && std::get<1> <= constraint_max
            {
                if (it != sit[location].begin() and
                        ...
                        std::get<2>(*std::prev(it))) // we can merge the current interval with the previous one
                {
                    if (it != sit[location].end() and std::next(it) != sit[location].end() and
                        ...
                        i_max == std::get<0>(*std::next(it)) and
                            std::get<2>(*std::next(it))) // we can merge the current interval with the next one
                    {
                        (*std::prev(it)) = std::make_tuple(std::get<0>(*std::prev(it)), std::get<1>(*std::next(it)), true);
                        sit[location].erase(std::next(it));
                        it = sit[location].erase(it);
                    } else {
                        (*std::prev(it)) = std::make_tuple(std::get<0>(*std::prev(it)), i_max, true);
                        it = sit[location].erase(it);
                    }
                    --it;
                } else {
                    if (it != sit[location].end() and std::next(it) != sit[location].end() and
                        ...
                        i_max == std::get<0>(*std::next(it)) and
                            std::get<2>(*std::next(it))) // we can merge the current interval with the next one
                    {
                        (*it) = std::make_tuple(i_min, std::get<1>(*std::next(it)), true);
                        sit[location].erase(std::next(it));
                    } else {
                        (*it) = std::make_tuple(i_min, i_max, true);
                    }
                }
            }
        }
...
/*void ReservationTable::mergeIntervals(list<Interval >& intervals) const
{
	...
	{
		if (get<1>(*prev) == std::get<0>(*curr) && std::get<2>(*prev) == std::get<2>(*curr))
		{
			*prev = make_tuple(get<0>(*prev), std::get<1>(*curr), std::get<2>(*prev));
...
// update SIT at the given location
    void ReservationTable::updateSIT(int location) {
        ...
        {
            if (constraint_table.length_min >
                constraint_table.length_max) // the location is blocked for the entire time horizon
            {
            ...
            sit[location].emplace_back(constraint_table.length_min, std::min(constraint_table.length_max + 1, MAX_TIMESTEP),
                                       false);
        } else {
            sit[location].emplace_back(0, std::min(constraint_table.length_max, MAX_TIMESTEP - 1) + 1, false);
        }
...
// return <upper_bound, low, high,  vertex collision, edge collision>
    std::list<std::tuple<int, int, int, bool, bool>>
    ReservationTable::get_safe_intervals(int from, int to, int lower_bound, int upper_bound) {
        std::list<std::tuple<int, int, int, bool, bool>> rst;
        ...
        for (auto interval : sit[to]) {
            ...
            auto t1 = get_earliest_arrival_time(from, to,
                                                std::max(lower_bound, std::get<0>(interval)), std::min(upper_bound, std::get<1>(interval)));
            if (t1 < 0) // the interval is not reachable
                continue;
            else if (std::get<2>(interval)) // the interval has collisions
            {
                rst.emplace_back(std::get<1>(interval), t1, std::get<1>(interval), true, false);
            ...
            { // so we need to check the move action has collisions or not
                ...
                if (t1 == t2)
                    rst.emplace_back(std::get<1>(interval), t1, std::get<1>(interval), false, false);
                else if (t2 < 0)
                    rst.emplace_back(std::get<1>(interval), t1, std::get<1>(interval), false, true);
                else {
                    rst.emplace_back(std::get<1>(interval), t1, t2, false, true);
                    rst.emplace_back(std::get<1>(interval), t2, std::get<1>(interval), false, false);
                }
            }
        }
        return rst;
...
// find a safe interval with t_min as given
    bool ReservationTable::find_safe_interval(Interval &interval, size_t location, int t_min) {
        if (t_min >= std::min(constraint_table.length_max, MAX_TIMESTEP - 1) + 1)
            return false;
        if (sit[location].empty())
            updateSIT(location);
    ...
    int ReservationTable::get_earliest_no_collision_arrival_time(int from, int to, const Interval &interval,
        ...
        for (auto t = std::max(lower_bound, std::get<0>(interval)); t < std::min(upper_bound, std::get<1>(interval)); t++) {
