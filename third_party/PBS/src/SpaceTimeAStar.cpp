...
namespace PBS_Li {
    ...
    void SpaceTimeAStar::updatePath(const LLNode *goal, std::vector <CBS_Li::PathEntry> &path) {
    ...
    CBS_Li::MAPFPath SpaceTimeAStar::findOptimalPath(const std::set<int> &higher_agents, const std::vector<CBS_Li::MAPFPath *> &paths, int agent,
                                             const CBS_Li::ConstraintTable &initial_constraints) {
        optimal = true;
        ...
        auto start = new AStarNode(start_location, 0, std::max(holding_time, my_heuristic[start_location]), nullptr, 0, 0);
        ...
        while (!open_list.empty()) {
            ...
            for (int next_location : next_locations) {
                ...
                int next_h_val = std::max(holding_time - next_g_val, my_heuristic[next_location]);
                ...
                if (existing_next->getFVal() > next->getFVal() or  // it has smaller f value
                    (existing_next->getFVal() == next->getFVal() and existing_next->num_of_conflicts >
