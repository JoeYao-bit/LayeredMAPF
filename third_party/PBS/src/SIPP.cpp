...
namespace PBS_Li {
    ...
    void SIPP::updatePath(const LLNode *goal, std::vector <CBS_Li::PathEntry> &path) {
    ...
    CBS_Li::MAPFPath SIPP::findOptimalPath(const std::set<int> &higher_agents, const std::vector<CBS_Li::MAPFPath *> &paths, int agent, const CBS_Li::ConstraintTable &initial_constraints) {
        reset();

        // build constraint table
        auto t = clock();
        CBS_Li::ConstraintTable constraint_table(instance.num_of_cols, instance.map_size);
        for (int a : higher_agents) {
            constraint_table.insert2CT(*paths[a]);
        }
        //std::cout << " static_paths size " << initial_constraints.static_paths.size() << std::endl;
        for(const auto& previous_path : initial_constraints.static_paths) {
            constraint_table.insert2CT(previous_path);
        }
        ...
        CBS_Li::ReservationTable reservation_table(constraint_table, goal_location);

        CBS_Li::MAPFPath path;
        num_expanded = 0;
        num_generated = 0;
        ...
        auto start = new SIPPNode(start_location, 0, std::max(my_heuristic[start_location], holding_time), nullptr, 0,
                                  std::get<1>(interval), std::get<1>(interval), std::get<2>(interval), std::get<2>(interval));
        min_f_val = std::max(holding_time, (int) start->getFVal());
        ...
        while (!open_list.empty()) {
            ...
            for (int next_location : instance.getNeighbors(curr->location)) // move to neighboring locations
            {
                for (auto &i : reservation_table.get_safe_intervals(
                    ...
                    std::tie(next_high_generation, next_timestep, next_high_expansion, next_v_collision,
                        next_e_collision) = i;
                    // compute cost to next_id via curr node
                    int next_g_val = next_timestep;
                    int next_h_val = std::max(my_heuristic[next_location], curr->getFVal() - next_g_val);  // path max
                    if (next_g_val + next_h_val > reservation_table.constraint_table.length_max)
                        continue;
                    int next_conflicts = curr->num_of_conflicts +
                                         (int) curr->collision_v * std::max(next_timestep - curr->timestep - 1, 0) +
            ...
            if (curr->high_expansion == curr->high_generation and
                reservation_table.find_safe_interval(interval, curr->location, curr->high_expansion) and
                    std::get<0>(interval) + curr->h_val <= reservation_table.constraint_table.length_max) {
                auto next_timestep = std::get<0>(interval);
                int next_h_val = std::max(curr->h_val, curr->getFVal() - next_timestep);  // path max
                auto next_collisions = curr->num_of_conflicts +
                                       (int) curr->collision_v * std::max(next_timestep - curr->timestep - 1, 0) // wait time
                                       + (int) std::get<2>(interval);
                auto next = new SIPPNode(curr->location, next_timestep, next_h_val, curr, next_timestep,
                                         std::get<1>(interval), std::get<1>(interval), std::get<2>(interval), next_collisions);
    ...
    bool SIPP::dominanceCheck(SIPPNode *new_node) {
        ...
        for (auto &old_node : ptr->second) {
            if (old_node->timestep <= new_node->timestep and
                old_node->num_of_conflicts <= new_node->num_of_conflicts) { // the new node is dominated by the old node
                return false;
            ...
            { // delete the old node
                ...
                return true;
                                                                         ...
                                                                         old_node->high_expansion) { // intervals overlap --> we need to split the node to make them disjoint
