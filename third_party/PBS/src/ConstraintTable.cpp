#include "PBS/inc/ConstraintTable.h"
namespace PBS_Li {
    ...
    int ConstraintTable::getMaxTimestep() const // everything is static after the max timestep
    {
        ...
        if (length_max < MAX_TIMESTEP)
            rst = std::max(rst, length_max);
        if (!landmarks.empty())
            rst = std::max(rst, landmarks.rbegin()->first);
        return rst;
    }
    ...
    void ConstraintTable::insert2CT(const CBS_Li::MAPFPath &path) {
...
// build the conflict avoidance table
    void ConstraintTable::insert2CAT(int agent, const std::vector<CBS_Li::MAPFPath *> &paths) {
    ...
    void ConstraintTable::insert2CAT(const CBS_Li::MAPFPath &path) {
        if (cat.empty()) {
            cat.resize(map_size);
        ...
        cat_max_timestep = std::max(cat_max_timestep, (int) path.size() - 1);
    }


// return the location-time pairs on the barrier in an increasing order of their timesteps
    std::list <std::pair<int, int>> ConstraintTable::decodeBarrier(int x, int y, int t) const {
        std::list <std::pair<int, int>> rst;
        int x1 = x / num_col, y1 = x % num_col;
        int x2 = y / num_col, y2 = y % num_col;
        if (x1 == x2) {
            if (y1 < y2)
                for (int i = std::min(y2 - y1, t); i >= 0; i--) {
                    rst.emplace_back(x1 * num_col + y2 - i, t - i);
                }
            else
                for (int i = std::min(y1 - y2, t); i >= 0; i--) {
                    rst.emplace_back(x1 * num_col + y2 + i, t - i);
                }
        } else // y1== y2
        {
            if (x1 < x2)
                for (int i = std::min(x2 - x1, t); i >= 0; i--) {
                    rst.emplace_back((x2 - i) * num_col + y1, t - i);
                }
            else
                for (int i = std::min(x1 - x2, t); i >= 0; i--) {
                    rst.emplace_back((x2 + i) * num_col + y1, t - i);
                }
        }
        return rst;
    ...
    int ConstraintTable::getHoldingTime(int location, int earliest_timestep) const {
        ...
        for (auto landmark : landmarks) {
            if (landmark.second != location)
                rst = std::max(rst, (int) landmark.first + 1);
        }
