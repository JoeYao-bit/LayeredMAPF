...
namespace PIBT_2 {
    ...
    Paths::Paths(int num_agents) {
        std::vector<path_pathfinding::Path> tmp(num_agents, path_pathfinding::Path(0));
        paths = tmp;
        makespan = 0;
    }

    path_pathfinding::Path Paths::get(int i) const {
    ...
    void Paths::insert(int i, const path_pathfinding::Path &path) {
        const int paths_size = paths.size();
        if (!(0 <= i && i < paths_size)) halt("invalid index");
    ...
    void Paths::operator+=(const Paths &other) {
        ...
        if (makespan == 0) {  // empty
            ...
            std::vector<path_pathfinding::Path> new_paths(paths_size);
            for (int i = 0; i < paths_size; ++i) {
                if (paths[i].empty() || *(paths[i].end() - 1) != other.paths[i][0]) {
                    halt("invalid operation");
                }
                path_pathfinding::Path tmp;
                // former
...
// this func is used when updating makespan
    ...
    int Paths::countConflict(int id, const path_pathfinding::Path &path) const {
        int cnt = 0;
        int makespan = getMakespan();
        int num_agents = size();
        const int path_size = path.size();
