...
// -------------------------------
    ...
    void MAPF_Solver::makeLogBasicInfo(std::ofstream &log) {
        path_pathfinding::Grid *grid = reinterpret_cast<path_pathfinding::Grid *>(P->getG());
...
// -------------------------------
    ...
    path_pathfinding::Path MinimumSolver::getPathBySpaceTimeAstar(
    ...
    path_pathfinding::Path MAPF_Solver::getPrioritizedPath(
    ...
    void MAPF_Solver::updatePathTableWithoutClear(const int id, const path_pathfinding::Path &p,
                                                  const Paths &paths) {
        if (p.empty()) return;
