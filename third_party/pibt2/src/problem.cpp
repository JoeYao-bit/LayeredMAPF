...
namespace PIBT_2 {
    ...
    Problem::Problem(freeNav::DimensionLength *dim, const freeNav::IS_OCCUPIED_FUNC<2> &isoc,
                     const freeNav::Instances<2> &instance_sat, CBS_Li::ConstraintTable* ct,
                     int _max_timestep, int _max_comp_time,
                     std::mt19937 *_MT) {
        ct_ = ct;
        // set the grid map
        G = new path_pathfinding::Grid(); // yz: when to release resource ?
...
// MAPF
    ...
    MAPF_Instance::MAPF_Instance(const std::string &_instance)
        ...
        while (getline(file, line)) {
            ...
            if (std::regex_match(line, results, r_map)) {
                G = new path_pathfinding::Grid(results[1].str());
                continue;
            }
            // set agent num
    ...
    MAPF_Instance::MAPF_Instance(freeNav::DimensionLength *dim, const freeNav::IS_OCCUPIED_FUNC<2> &isoc,
                                 const freeNav::Instances<2> &instance_sat, CBS_Li::ConstraintTable* ct, int _max_timestep,
                                 int _max_comp_time, std::mt19937 *_MT)
                                 :Problem(dim, isoc, instance_sat, ct, _max_timestep, _max_comp_time, _MT),
    ...
    void MAPF_Instance::setRandomStartsGoals() {
        ...
        // get grid size
        path_pathfinding::Grid *grid = reinterpret_cast<path_pathfinding::Grid *>(G);
