#include "MAPF-LNS2/inc/InitLNS.h"
#include <queue>
#include <algorithm>
...
namespace MAPF_LNS {
    ...
    InitLNS::InitLNS(const Instance &instance, vector<Agent> &agents, double time_limit,
            ...
            path_table(instance.map_size, agents.size()), collision_graph(agents.size()),
...
// return true if the new p[ath has collisions;
    ...
    bool InitLNS::generateNeighborByTarget() {
        ...
        agents[a].path_planner->findMinimumSetofColldingTargets(goal_table,
                                                                A_target);// generate non-wait path and collect A_target
