	...
	list<shared_ptr<Conflict>>& copy, int excluded_agent) const
{
    ...
    void CBS::printResults() const {
        ...
        cout << solution_cost << "," << runtime << "," <<
             num_HL_expanded << ","
             << // num_LL_expanded << "," << HL_num_generated << "," << LL_num_generated << "," <<
    ...
    CBS::CBS(const Instance &instance, bool sipp, int screen, CBS_Li::ConstraintTable *ct) :
            screen(screen), suboptimality(1),
    ...
    bool CBS::generateRoot() {
        ...
        if (paths_found_initially.empty()) {
            ...
            for (auto i : agents) {
                ...
                paths_found_initially[i] = search_engines[i]->findOptimalPath(*root, initial_constraints[i], paths, i,
                                                                              0);
