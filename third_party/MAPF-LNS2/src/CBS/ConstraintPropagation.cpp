...
// is a mutex edge mutex.
    ...
    bool ConstraintPropagation::has_mutex(edge_pair e) {
        return (bwd_mutexes.find({e.first, e.second}) != bwd_mutexes.end() ||
                bwd_mutexes.find({e.second, e.first}) != bwd_mutexes.end()) || has_fwd_mutex(e);
    }
...
// }
    ...
    std::pair<std::vector<Constraint>, std::vector<Constraint>>
    ConstraintPropagation::generate_constraints(int level_0, int level_1) {
        MDD *mdd_s = mdd0;
        MDD *mdd_l = mdd1;
        bool reversed = false;
        if (level_0 > level_1) {
