                                 ...
                                 child[i]->distance_to_go < curr->distance_to_go) // Bypass1
                        {
                            ...
                            for (const auto &path : child[i]->paths) {
                                ...
                                if ((double) path.second.first.size() - 1 >
                                    suboptimality * fmin_copy[path.first]) // Our bypassing
                                {
                                    foundBypass = false;
                                    break;
                                }
...
// also, do the same for ll_min_f_vals and paths_costs (since its already "on the way").
    ...
    bool ECBS::findPathForSingleAgent(ECBSNode *node, int ag) {
        clock_t t = clock();
        auto new_path = search_engines[ag]->findSuboptimalPath(*node, initial_constraints[ag], paths, ag,
                                                               min_f_vals[ag], suboptimality);
