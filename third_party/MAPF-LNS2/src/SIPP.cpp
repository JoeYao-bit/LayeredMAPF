...
// Returns a path that minimizes the collisions with the paths in the path table, breaking ties by the length
    Path SIPP::findPath(const ConstraintTable &constraint_table) {
        ...
        //if (!path.empty())
        //    return path;
        // yz: add static path as constraint
        ConstraintTable constraint_table_copy(constraint_table);
        if(ct != nullptr) {
            for(const auto& previous_path : ct->static_paths) {
                Path path_temp;
                for(const auto& prev_pt : previous_path) {
                    path_temp.emplace_back(prev_pt.location);
                }
                constraint_table_copy.insert2CT(path_temp);
            }
        }
        ReservationTable* reservation_table = nullptr;
        if(ct != nullptr) {
            reservation_table = new ReservationTable(constraint_table_copy, goal_location);
        } else {
            reservation_table = new ReservationTable(constraint_table, goal_location);
        }
        Path path;
        ...
        //}
        releaseNodes();
        if(reservation_table != nullptr) {
            delete reservation_table;
            reservation_table = nullptr;
        }
        return path;
    }
    ...
    pair<Path, int> SIPP::findSuboptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
        ...
        if(ct != nullptr) {
            for(const auto& previous_path : ct->static_paths) {
                ...
                constraint_table.insert2CT(path_temp);
            }
        }
...
// TODO:: currently this is implemented in SIPP inefficiently
    int SIPP::getTravelTime(int start, int end, const ConstraintTable &constraint_table, int upper_bound) {
        ...
        if(ct != nullptr) {
            static_timestep = max(static_timestep, ct->getMaxTimestep());
        }
        while (!open_list.empty()) {
