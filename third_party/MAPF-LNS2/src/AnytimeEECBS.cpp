...
namespace MAPF_LNS {
    ...
    void AnytimeEECBS::run() {
        ...
        //ECBS ecbs(instance, false, screen, ct_);
        CBS_Li::ECBS ecbs(instance, *ct_, false, screen);
        ...
        ecbs.setHeuristicType(CBS_Li::heuristics_type::WDG, CBS_Li::heuristics_type::GLOBAL);
        ecbs.setTargetReasoning(true);
        ecbs.setMutexReasoning(false);
        ecbs.setConflictSelectionRule(CBS_Li::conflict_selection::EARLIEST);
        ecbs.setNodeSelectionRule(CBS_Li::node_selection::NODE_CONFLICTPAIRS);
        ...
        while (runtime < time_limit && sum_of_costs > sum_of_costs_lowerbound) {
            ecbs.clear();
            ecbs.setHighLevelSolver(CBS_Li::high_level_solver_type::EES, w);
    ...
    void AnytimeEECBS::savePaths() { // yz: save result path in freeNav style
        fr_paths_.clear();
...
//        }
        for (const auto & raw_path : solution) {
            freeNav::Path<2> path;
            for (const auto &state : raw_path) {
                freeNav::Pointi<2> pt;
                pt[1] = instance.getRowCoordinate(state.location);
                pt[0] = instance.getColCoordinate(state.location);
                path.push_back(pt);
            }
            fr_paths_.push_back(path);
        }
