...
namespace CBS_Li {
    ...
    typedef std::tuple<int, int, bool> Interval; // [t_min, t_max), num_of_collisions

// yz: for get safe interval in SIPP
    ...
    public:
        ...
        list<std::tuple<int, int, int, bool, bool> > get_safe_intervals(int from, int to, int lower_bound, int upper_bound);
    ...
    private:
        ...
        SIT sit; // location -> [t_min, t_max), num_of_collisions // yz: number of collision for each grid
