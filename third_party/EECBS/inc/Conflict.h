...
// We prioritize them over normal semi-/non-caridnal conflicts 
    ...
    enum conflict_selection {
        RANDOM, EARLIEST, CONFLICTS, MCONSTRAINTS, FCONSTRAINTS, WIDTH, SINGLETONS
    };

// 1: agent id; 2: vertex id; 3: vertex id (for edge constraint); 4: time index
...
// <agent, loc, -1, t, GLENGTH>: path of agent_id should be of length at least t + 1
    ...
    class Conflict {
    public:
        int a1; // the first agent
        int a2; // the second agent
        list<Constraint> constraint1; // list to store multiple constraint in rectangle
        ...
        // add constraint that forbid agent pass the vertex
        ...
        // add constraint that forbid agent follow the edge
        ...
        bool rectangleConflict(int a1, int a2, const std::pair<int, int> &Rs, const std::pair<int, int> &Rg,
                               int Rg_t, const list<Constraint> &constraint1,
