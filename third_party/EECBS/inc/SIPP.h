...
// yz: SIPP：安全间隔路径规划：SIPP: Safe Interval Path Planning for Dynamic Environments
    ...
    public:
        ...
        MAPFPath findOptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
                                 const vector<MAPFPath *> &paths, int agent, int lowerbound);

        pair<MAPFPath, int> findSuboptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
                                               const vector<MAPFPath *> &paths, int agent, int lowerbound,
                                               double w);  // return the path and the lowerbound
        MAPFPath findPath(
                const ConstraintTable &constraint_table); // return A path that minimizes collisions, breaking ties by cost
