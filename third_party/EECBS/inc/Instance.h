...
// load map, agents, start and target
    class Instance {
    public:
        int num_of_cols;
        int num_of_rows;
        ...
        Instance(const string &map_fname, const string &agent_fname,
                 int num_of_agents = 0, int num_of_rows = 0, int num_of_cols = 0, int num_of_obstacles = 0,
...
//private:
        ...
        std::vector<bool> my_map; // vector to store the map
        string map_fname; // file name
        string agent_fname; // agent name
        ...
        bool isConnected(int start,
                         int goal); // run BFS to find a path between start and goal, return true if a path exists.
