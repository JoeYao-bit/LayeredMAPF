...
namespace CBS_Li {
    ...
    public:
        ...
        void
        insert2CT(const HLNode &node, int agent); // build the constraint table for the given agent at the give node
        void insert2CT(const list<Constraint> &constraints,
                       int agent); // insert constraints for the given agent to the constraint table
        void insert2CT(const MAPFPath &path); // insert a path to the constraint table
        ...
        void insert2CAT(int agent,
                        const vector<MAPFPath *> &paths); // build the conflict avoidance table using a set of paths
        void insert2CAT(const MAPFPath &path); // insert a path to the collision avoidance table
        //int getCATMaxTimestep() const {return cat_max_timestep;}

        std::vector<MAPFPath> static_paths; // yz: used for layered mapf

//protected:
        friend class ReservationTable;

        // yz: size_t : edge/node id, int, int : min time and max time, time range
        ...
        CAT cat; // store collision state of grid map, depth is time range
        int cat_max_timestep = 0;
        vector<int> cat_goals; // the visit time of goal
        // yz: location at time stamp must be occupied, seems not related to agent, maybe can only be occupied one agent
        // int: time index, size_t: grid id that must be occupied
        ...
        // determine the edge id, view from and to as two dimension
