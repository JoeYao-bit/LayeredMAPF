...
namespace CBS_Li {
    ...
    public:
        ...
        HLNode *getGoalNode() { return goal_node; }
        ...
        void
        setBypass(bool b) { bypass = b; } // 2-agent solver for heuristic calculation does not need bypass strategy.
        ...
        CBS(const Instance &instance, const ConstraintTable &static_constraint, bool sipp, int screen);
        ...
        void savePaths(const string &fileName); // write the paths to a file
        void savePaths();
        ...
        std::vector<MAPFPath *> getPath() { return paths; }

        freeNav::Paths<2> getfreeNavPath() { return fr_paths_; }

        //vector<ConstraintTable> initial_constraints; // yz: aborted

        const ConstraintTable &static_constraint; // yz: use only to save space
    ...
    //protected:
        ...
        bool target_reasoning;  // using target reasoning 目标推理
        ...
        int cost_lowerbound = 0; // yz: minimum estimation of conflict free optimal result, update after each expansion of hyper node
        ...
        vector<MAPFPath *> paths;
        vector<MAPFPath> paths_found_initially;  // contain initial paths found
        // vector<MDD*> mdds_initially;  // contain initial paths found
        // yz: allocate a single solver for each agent
        ...
        std::shared_ptr<Conflict> chooseConflict(const HLNode &node) const;

        static void copyConflicts(const list<std::shared_ptr<Conflict>> &conflicts,
                                  list<std::shared_ptr<Conflict>> &copy, const list<int> &excluded_agent);

        void removeLowPriorityConflicts(list<std::shared_ptr<Conflict>> &conflicts) const;
        ...
        void computeConflictPriority(std::shared_ptr<Conflict> &con,
                                     CBSNode &node); // check the conflict is cardinal, semi-cardinal or non-cardinal

        //update information
        inline void updatePaths(HLNode *curr);
