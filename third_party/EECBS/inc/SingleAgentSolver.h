...
// yz: find optimal path under constraints
    ...
    public:
        ...
        virtual MAPFPath findOptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
                                         const vector<MAPFPath *> &paths, int agent, int lower_bound) = 0;

        virtual pair<MAPFPath, int> findSuboptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
                                                       const vector<MAPFPath *> &paths, int agent, int lowerbound,
                                                       double w) = 0;  // return the path and the lowerbound
