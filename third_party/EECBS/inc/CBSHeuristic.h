...
// yz: enable HLNode work with hasher table
...
// <h value, a1 f at root, a2 f at root> for ECBS
    typedef boost::unordered_map<HTableEntry, std::tuple<int, int, int>, HTableEntry::Hasher, HTableEntry::EqNode> HTable;
    ...
    public:
        ...
        //stats
        list<std::tuple<int, int, const HLNode *, uint64_t, int> > sub_instances;    // <agent 1, agent 2, node, number of expanded CT nodes, h value>


        CBSHeuristic(int num_of_agents,
                     const vector<MAPFPath *> &paths,
        ...
        void setInadmissibleHeuristics(heuristics_type h) {
            ...
            if (h == heuristics_type::CONFLICT) {
                sum_distance_errors.assign(int(conflict_type::TYPE_COUNT),
                                           0);  //(int(conflict_priority::PRIORITY_COUNT) * int(conflict_type::TYPE_COUNT), 0);
        ...
        double getCostError(int i = 0) const {
            return (num_of_errors[i] == 0) ? 0 : sum_cost_errors[i] / num_of_errors[i];
        }

        double getDistanceError(int i = 0) const {
            return (num_of_errors[i] == 0) ? 0 : sum_distance_errors[i] / num_of_errors[i];
        }
    ...
    private:
        ...
        int screen = 0;
        int num_of_agents;
        std::vector<std::vector<HTable> > lookupTable; // yz: all pair of agents, and high level nodes of a pair

        // double sum_distance_error = 0;
        // double sum_cost_error = 0;
        // int num_of_errors = 0;
        // int initialize_online_learning = 0;
        std::vector<double> sum_distance_errors;
        std::vector<double> sum_cost_errors;
        std::vector<int> num_of_errors;
        ...
        pair<int, int>
        solve2Agents(int a1, int a2, const CBSNode &node, bool cardinal); // return h value and num of CT nodes
        std::tuple<int, int, int> solve2Agents(int a1, int a2, const ECBSNode &node); // return h value and num of CT nodes
        ...
        int DPForWMVC(vector<int> &x, int i, int sum, const vector<int> &CG, const vector<int> &range,
                      int &best_so_far); // dynamic programming
        ...
        int DPForConstrainedWMVC(vector<bool> &x, int i, int sum, const vector<int> &CG, const vector<int> &range,
                                 int &best_so_far);
    };
