...
namespace CBS_Li {
    ...
    public:
        ...
        std::shared_ptr<Conflict> run(const vector<MAPFPath *> &paths, int timestep,
    ...
    private:
        ...
        std::shared_ptr<Conflict> findRectangleConflictByRM(const vector<MAPFPath *> &paths, int timestep,
                                                       int a1, int a2, const MDD *mdd1, const MDD *mdd2);

        std::shared_ptr<Conflict> findRectangleConflictByGR(const vector<MAPFPath *> &paths, int timestep,
                                                       int a1, int a2, const MDD *mdd1, const MDD *mdd2);

        bool
        ExtractBarriers(const MDD &mdd, int loc, int timestep, int dir, int dir2, int start, int goal, int start_time,
                        list<Constraint> &B);
        ...
        void generalizedRectangle(const vector<PathEntry> &path1, const vector<PathEntry> &path2, const MDD &mdd1,
                                  const MDD &mdd2,
        ...
        bool isRectangleConflict(const pair<int, int> &s1, const pair<int, int> &s2, const pair<int, int> &g1,
        ...
        int classifyRectangleConflict(const pair<int, int> &s1, const pair<int, int> &s2, const pair<int, int> &g1,
                                      const pair<int, int> &g2, const pair<int, int> &Rg);// for RM
        ...
        list<int> getStartCandidates(const MAPFPath &path, const MDD &mdd, int timestep);

        list<int> getGoalCandidates(const MAPFPath &path, const MDD &mdd, int timestep);

        //Compute start and goal candidates for GR
        int getStartCandidate(const MAPFPath &path, int dir1, int dir2, int timestep);

        int getGoalCandidate(const MAPFPath &path, int dir1, int dir2, int timestep);
        ...
        bool blocked(const MAPFPath &path, const list<Constraint> &constraints);

        bool traverse(const MAPFPath &path, int loc, int t);
