...
// also, do the same for ll_min_f_vals and paths_costs (since its already "on the way").
    ...
    bool ECBS::generateRoot() {
        ...
        //std::cout << " suboptimality " << suboptimality << std::endl;
        for (auto i : agents) {
            // yz: replace is okay
            //paths_found_initially[i] = search_engines[i]->findSuboptimalPath(*root, initial_constraints[i], paths, i, 0, suboptimality);
            paths_found_initially[i] = search_engines[i]->findSuboptimalPath(*root, static_constraint, paths, i, 0,
                                                                             suboptimality);
            if (paths_found_initially[i].first.empty()) {
                int start_id = search_engines[i]->start_location;
                int target_id = search_engines[i]->goal_location;
                freeNav::Pointi<2> start_pt = {search_engines[i]->instance.getColCoordinate(start_id),
...
//                cout << " start id " << start_id << " target id " << target_id << std::endl;
    ...
    bool ECBS::findPathForSingleAgent(ECBSNode *node, int ag) {
        ...
        auto new_path = search_engines[ag]->findSuboptimalPath(*node, static_constraint, paths, ag, min_f_vals[ag],
                                                               suboptimality);
        ...
        while (!node.unknownConf.empty()) {
            ...
            if (PC)
                if (node.chosen_from == "cleanup" ||
                    ...
                    (int) paths[a2]->size() - 1 ==
                    min_f_vals[a2]) // the path of at least one agent is its shortest path
