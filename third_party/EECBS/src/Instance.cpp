    ...
    void Instance::generateRandomAgents(int warehouse_width) {
        ...
        if (warehouse_width == 0)//Generate agents randomly
        {
            ...
            while (k < num_of_agents) {
                ...
                if (my_map[start] || starts[start]) // jump if start is occupied or has been select as start
                    continue;
                ...
                while (my_map[goal] || goals[goal]) // jump if target is occupied or has been select as target
...
// node transfer validity check, four nearest neighbor, next must be free
...
// add obstacle, but do not hurt the connection between nearby free grids
    bool Instance::addObstacle(int obstacle) {
        ...
        int start = 0;
        int goal = 1;
        // start and goal is diagonal
        while (start < 3 && goal < 4) {
            // if current start is illegal (out of boundary / occupied)
            if (x[start] < 0 || x[start] >= num_of_rows || y[start] < 0 || y[start] >= num_of_cols
                ...
                // if current target is illegal (out of boundary / occupied)
                                   ...
                                   linearizeCoordinate(x[goal], y[goal]))) // cannot find a path from start to goal
            {
                ...
                //  if two free nearby grid is not connect, cannot set current grid to obstacle
...
// check whether two node are connect, in a Breadth First way
...
// make sure it's a full-connect graph
    void Instance::generateConnectedRandomGrid(int rows, int cols, int obstacles) {
        ...
        // add padding, set the boundary line to occupied
        i = 0;
        ...
        // add obstacles uniformly at random, and make sure its full-connected
        i = 0;
        while (i < obstacles) {
...
// yz: get the first num_of_agents test cases
...
//    freeNav::Instances<2> partly_ist;
//    std::vector<int> partly_id = {1,3,10,12,13,14,15,19,20,24,25,27,31,33,34,38,39,40,46,47,51,52,56,58,59,63,64,68};
//    std::vector<int> new_start, new_goal;
//    for(const int& local_id : partly_id) {
//        new_start.push_back(start_locations[local_id]);
//        new_goal.push_back(goal_locations[local_id]);
//    }
//    num_of_agents = partly_id.size();
//    start_locations = new_start;
//    goal_locations = new_goal;
        myfile.close();
        return true;
...
// yz: nearest four passable neighbors
