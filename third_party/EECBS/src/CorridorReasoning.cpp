...
namespace CBS_Li {
                                        ...
                                        int endpoints_time[]) // return the length of the corridor
    {
        ...
        endpoints_time[0] = getExitingTime(*paths[conflict->a1],
                                           t);; // the first timestep when agent 1 exits the corridor
        endpoints_time[1] = getExitingTime(*paths[conflict->a2],
                                           t);; // the first timestep when agent 2 exits the corridor
    ...
    shared_ptr<Conflict> CorridorReasoning::findCorridorConflict(const shared_ptr<Conflict> &conflict,
        ...
        int t3, t3_, t4, t4_;
        // yz: replace is okay
        ConstraintTable ct1(static_constraint);
        //ConstraintTable ct1(initial_constraints[conflict->a1]);
        ct1.insert2CT(node, conflict->a1);
        t3 = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front().location, u[1], ct1,
                                                         MAX_TIMESTEP);
        ...
        t3_ = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front().location, u[1], ct1,
                                                          t3 + 2 * corridor_length + 1);
        ConstraintTable ct2(static_constraint);
        //ConstraintTable ct2(initial_constraints[conflict->a2]);
        ct2.insert2CT(node, conflict->a2);
        t4 = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front().location, u[0], ct2,
                                                         MAX_TIMESTEP);
        ...
        t4_ = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front().location, u[0], ct2,
                                                          t3 + corridor_length + 1);
...
*/
    ...
    bool CorridorReasoning::blocked(const MAPFPath &path, const Constraint &constraint) {
        int a, loc, t1, t2;
        constraint_type type;
