        ...
        for (auto it = sit[location].begin(); it != sit[location].end(); ++it) {
            ...
            if (i_min < t_min && i_max <= t_max) {
                if (it != sit[location].end() and std::next(it) != sit[location].end() and
                    ...
                    i_max == get<0>(*std::next(it)) and
                    get<2>(*std::next(it))) // we can merge the current interval with the next one
                {
                ...
                if (it != sit[location].begin() and
                    ...
                    i_min == get<1>(*std::prev(it)) and
                    get<2>(*std::prev(it))) // we can merge the current interval with the previous one
                {
...
// yz: initialize a grid / location's interval
    void ReservationTable::updateSIT(int location) {
            ...
            goal_location) // we need to divide the same intervals into 2 parts [0, length_min) and [length_min, length_max + 1)
        {
            ...
            {
                sit[location].emplace_back(0, 0, false); // yz: false means safe while true means collision
                return;
            }
        ...
        if (!constraint_table.cat.empty()) {
            for (auto t = 0; t < constraint_table.cat[location].size(); t++) {
                // yz: if grid location is occupied at timestamp t, insert collision interval
                if (constraint_table.cat[location][t])
                    insertSoftConstraint2SIT(location, t, t + 1);
            }
            // yz: if grid location is occupied by goal, insert collision interval that end at infinity
...
// yz: return safe intervals between (lower_bound, upper_bound)
    ...
    ReservationTable::get_safe_intervals(int from, int to, int lower_bound, int upper_bound) {
        ...
        // yz: for all intervals to 'to'
        for (auto interval : sit[to]) {
            // yz: if lower bound > upper bound of interval
            if (lower_bound >= get<1>(interval))
                continue;
                // yz: if upper bound < lower bound of interval
    ...
    Interval ReservationTable::get_first_safe_interval(size_t location) {
        if (sit[location].empty())
            updateSIT(location);
        return sit[location].front(); // yz: the first interval is always safe interval, determined by updateSIT(...)
    }
