...
/*bool MDD::isConstrained(int curr_id, int next_id, int next_timestep, const std::vector< std::list< std::pair<int, int> > >& cons)  const
{
    ...
    bool MDD::buildMDD(ConstraintTable &constraint_table, const SingleAgentSolver *_solver) {
        ...
        int holding_time = constraint_table.getHoldingTime(solver->goal_location,
                                                           constraint_table.length_min); // the earliest timestep that the agent can hold its goal location. The length_min is considered here.
        ...
        while (!open.empty()) {
            ...
            {
                if (curr->parents.size() != 1 || curr->parents.front()->location !=
                                                 solver->goal_location) { // skip the case where curr only have parent node who locates at goal_location
                    goal_node = curr;
...
// yz: find node at level and location
    ...
    MDD::MDD(const MDD &cpy) // deep copy
    {
        ...
        for (size_t t = 0; t < levels.size() - 1; t++) {
            for (auto node = levels[t].begin(); node != levels[t].end(); ++node) {
                ...
                for (list<MDDNode *>::const_iterator cpyChild = cpyNode->children.begin();
...
// yz: search goal at specific level
...
// yz: traversal current level to find specific location
    ...
    MDD *MDDTable::getMDD(HLNode &node, int id, size_t mdd_levels) {
        ...
        clock_t t = clock();
        MDD *mdd = new MDD();
        ConstraintTable ct(static_constraint);
