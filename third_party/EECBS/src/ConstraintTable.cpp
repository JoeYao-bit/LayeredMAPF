...
// get the last collide time index of a grid
...
// add constraints for the given agent
    void ConstraintTable::insert2CT(const list<Constraint> &constraints, int agent) {
        ...
        // a: agent / x: first vertext / y : second vertex / t: time index / type: constraint type
            ...
            case constraint_type::BARRIER:
                if (a == agent) {
                    for (auto constraint : constraints) {
                        ...
                        // yz: a barrier is a line of grids that cannot be pass at the timerange
...
// yz: add path as constraint, other path cannot have vertex or edge overlap with this one
// prevent any agent into this zone
    void ConstraintTable::insert2CT(const MAPFPath &path) {
...
// yz: landmarks means location loc should have agent at time stamp t
...
// build the conflict avoidance table
    void ConstraintTable::insert2CAT(int agent, const vector<MAPFPath *> &paths) {
...
// yz: set a path as constraint, avoid other have conflict with this path
    void ConstraintTable::insert2CAT(const MAPFPath &path) {
        if (cat.empty()) {
            cat.resize(map_size);
...
// return the location-time pairs on the barrier in an increasing order of their timestamps
// yz: set obstacle between two vertex id, at timestamp t
    list<pair<int, int> > ConstraintTable::decodeBarrier(int x, int y, int t) const {
        list<pair<int, int> > rst;
        // yz: get coordinates of two vertex
        int x1 = x / num_col, y1 = x % num_col;
        int x2 = y / num_col, y2 = y % num_col;
        // yz: seems assume have the same x value or the same y value
        if (x1 == x2) {
            if (y1 < y2)
...
// whether vertex loc at time stamp t
    bool ConstraintTable::constrained(size_t loc, int t) const {
        assert(loc >= 0);
        // yz: whether vertex loc must be occupied at time stamp t
        if (loc < map_size) {
    ...
    int ConstraintTable::getNumOfConflictsForStep(size_t curr_id, size_t next_id, int next_timestep) const {
        int rst = 0;
        if (!cat.empty()) {
            // yz: whether next grid is visited and occupied
            // and current is not in conflict
            if (cat[next_id].size() > next_timestep and cat[next_id][next_timestep])
                rst++;
            //  yz: whether there is edge conflict such as curr -> next and curr <- next
            if (curr_id != next_id and cat[next_id].size() >= next_timestep and cat[curr_id].size() > next_timestep and
                ...
                rst++;
            // yz: whether if grid have been occupied by goal
    ...
    bool ConstraintTable::hasConflictForStep(size_t curr_id, size_t next_id, int next_timestep) const {
        // yz: if there is no occupied edge or vertex, return no conflict
        if (!cat.empty()) {
            // yz: whether next grid is visited and occupied
            // and current is not in conflict
            if (cat[next_id].size() > next_timestep and cat[next_id][next_timestep])
...
// yz: get the total count of future collision
...
// return the earliest timestep that the agent can hold the location
    int ConstraintTable::getHoldingTime(int location, int earliest_timestep) const {
        ...
        // Landmark, must be occupied at the time stamp
