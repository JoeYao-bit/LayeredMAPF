...
// yz: calculate cost_to_go for a HLNode
    void CBSHeuristic::updateInadmissibleHeuristics(HLNode &curr) {
        ...
        switch (inadmissible_heuristic) {
            ...
            case heuristics_type::LOCAL:
                // yz: cost_to_go determined by count of conflict + conflict agents
                if (std::abs(1 - sum_distance_errors[0]) < 0.001)
                    curr.cost_to_go = h + max(0, (int) (curr.distance_to_go) * 1000);
                else
                    curr.cost_to_go =
                            h + max(0, (int) (curr.distance_to_go * sum_cost_errors[0] / (1 - sum_distance_errors[0])));
        ...
        if ((inadmissible_heuristic == heuristics_type::GLOBAL ||
            ...
            if (curr.parent->fully_expanded) // update error
            {
                ...
                } else if (inadmissible_heuristic == heuristics_type::LOCAL) // update local error
                {
                    ...
                    sum_distance_errors[0] =
                            sum_distance_errors[0] * (1 - learning_rate) + curr.parent->distance_error * learning_rate;
                    sum_cost_errors[0] =
                            sum_cost_errors[0] * (1 - learning_rate) + curr.parent->cost_error * learning_rate;
    ...
    void CBSHeuristic::updateOnlineHeuristicErrors(ECBSNode &parent) {
        ...
        } else if (inadmissible_heuristic == heuristics_type::LOCAL) // update local error
        {
            ...
            sum_distance_errors[0] =
                    sum_distance_errors[0] * (1 - learning_rate) + parent.distance_error * learning_rate;
...
// yz: calculate heuristic value for hyper node
    bool CBSHeuristic::computeInformedHeuristics(CBSNode &curr, double _time_limit) {
        ...
        switch (type) {
            ...
            case heuristics_type::CG:
                ...
                if (curr.parent == nullptr || num_of_CGedges > ILP_edge_threshold ||
                    // root node of CBS tree or the graph is too large
                    target_reasoning ||
                    disjoint_splitting) // when we are allowed to replan for multiple agents, the incremental method is not correct any longer.
...
// yz: if a hyper have a cardinal conflict, them the two agent are dependency each pother
    ...
    bool CBSHeuristic::buildDependenceGraph(CBSNode &node, vector<int> &CG, int &num_of_CGedges) {
        ...
        for (auto &conflict : node.conflicts) {
            // yz: sort a1,a2 id in increasing order
    ...
    bool CBSHeuristic::buildWeightedDependencyGraph(ECBSNode &node, const vector<int> &min_f_vals, vector<int> &CG,
                                                    int &delta_g) {
        delta_g = 0;
        ...
        for (const auto &conflict : node.conflicts) {
            ...
            if (got != lookupTable[a1][a2].end()) // check the lookup table first
            {
                ...
                if (!counted[a1]) {
                    ...
                    if (get<1>(got->second) < min_f_vals[a1]) { return false; }
                    delta_g += get<1>(got->second) - min_f_vals[a1];
                    counted[a1] = true;
                }
                if (!counted[a2]) {
                    //assert(get<2>(got->second) >= min_f_vals[a2]);
                    if (get<2>(got->second) < min_f_vals[a2]) { return false; }
                ...
                if (!counted[a1]) {
                    ...
                    if (get<1>(rst) < min_f_vals[a1]) { return false; }
                    delta_g += get<1>(rst) - min_f_vals[a1];
                    counted[a1] = true;
                }
                if (!counted[a2]) {
                    //assert(get<2>(rst) >= min_f_vals[a2]);
                    if (get<2>(rst) < min_f_vals[a2]) { return false; }
...
//	cbs.setMutexReasoning(mutex_reasoning);
        ...
        if (cbs.runtime >= time_limit - runtime) // time out
            ...
            return make_tuple(cbs.getLowerBound() - cbs.dummy_start->g_val,
                              cbs.getInitialPathLength(0),
                              cbs.getInitialPathLength(1)); // using lowerbound to approximate
        else if (cbs.solution_cost < 0) // no solution
...
/*void CBSHeuristic::setUpSubSolver(CBS& cbs) const
{
    ...
    int CBSHeuristic::minimumVertexCover(const vector<int> &CG) {
        clock_t t = clock();
        int rst = 0;
        std::vector<bool> done(num_of_agents, false); // yz: whether the agent is allocate to a subgraph
        for (int i = 0; i < num_of_agents; i++) {
            if (done[i])
                continue;
            std::vector<int> indices;
            ...
            // yz: detect a local dependent sub graph
            while (!Q.empty()) {
                int j = Q.front();
                Q.pop();
                indices.push_back(j); // yz: add vertex to the list
                for (int k = 0; k < num_of_agents; k++) {
                    // yz: if found a pair of dependent agents. / found an edge
            ...
            // yz: agent connect graph (edge graph)
            std::vector<int> subgraph(indices.size() * indices.size(), 0);
            int num_edges = 0; // yz: that number of agent that
            ...
            if (num_edges > ILP_edge_threshold) {
                // yz: count pair of agent that have cardinal conflict. in the subgraph
...
// yz: HG, curr.parent->h_val, num_of_agents, num_of_CGedges
...
//  CG, num_of_CGnodes, num_of_CGedges, i, cols
    bool
    CBSHeuristic::KVertexCover(const std::vector<int> &CG, int num_of_CGnodes, int num_of_CGedges, int k, int cols) {
        double runtime = (double) (clock() - start_time) / CLOCKS_PER_SEC;
        if (runtime > time_limit)
...
// yz: count edge > 0 (pairs that have cardinal conflict) in subgraph
//     count edge that both vertex not in used list
...
// enumerate all possible assignments and return the best one
    int CBSHeuristic::weightedVertexCover(const std::vector<int> &CG) {
        ...
        for (int i = 0; i < num_of_agents; i++) {
            ...
            {
                rst += std::max(CG[indices[0] * num_of_agents + indices[1]],
            ...
            for (int j = 0; j < num; j++) {
                for (int k = j + 1; k < num; k++) {
                    G[j * num + k] = std::max(CG[indices[j] * num_of_agents + indices[k]],
...
// recusive component of weighted vertex cover
    int CBSHeuristic::DPForConstrainedWMVC(std::vector<bool> &x, int i, int sum, const std::vector<int> &CG,
                                           const std::vector<int> &range, int &best_so_far) {
        if (sum >= best_so_far)
            return INT_MAX;
        double runtime = (double) (clock() - start_time) / CLOCKS_PER_SEC;
