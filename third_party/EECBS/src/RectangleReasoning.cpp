...
namespace CBS_Li {
    ...
    shared_ptr<Conflict> RectangleReasoning::run(const vector<MAPFPath *> &paths, int timestep,
    ...
    shared_ptr<Conflict> RectangleReasoning::findRectangleConflictByGR(const vector<MAPFPath *> &paths, int timestep,
                                                                       int a1, int a2, const MDD *mdd1,
                                                                       const MDD *mdd2) {
        assert(timestep > 0);
        ...
        bool haveBarriers = ExtractBarriers(*mdd1, loc, timestep, loc - from1, loc - from2,
                                            paths[a1]->at(t_start).location, paths[a1]->at(t_end).location, t_start,
                                            B1);
        if (!haveBarriers)
            return nullptr;
        ...
        bool succ;
        if (abs(loc - from1) == 1 ||
            abs(loc - from2) > 1) // first agent moves horizontally and second agent moves vertically
        {
    ...
    shared_ptr<Conflict> RectangleReasoning::findRectangleConflictByRM(const vector<MAPFPath *> &paths, int timestep,
                                                                       int a1, int a2, const MDD *mdd1,
                                                                       const MDD *mdd2) {
    ...
    bool RectangleReasoning::ExtractBarriers(const MDD &mdd, int loc, int timestep,
                                             int dir1, int dir2, int start, int goal, int start_time,
                                             list<Constraint> &B) {
        int num_barrier;
        ...
        for (size_t t = 1; t < mdd.levels.size(); t++) {
            for (auto n : mdd.levels[t]) {
                ...
                if (abs(dir1) == 1) //vertical barriers
                {
                    ...
                    barrier_time =
                            timestep + sign1 * (instance.getColCoordinate(n->location) - instance.getColCoordinate(loc))
                            + sign2 * (instance.getRowCoordinate(n->location) - instance.getRowCoordinate(loc));
                } else {
                    barrier_id = sign1 * (instance.getRowCoordinate(n->location) - instance.getRowCoordinate(start));
                    barrier_time =
                            timestep + sign1 * (instance.getRowCoordinate(n->location) - instance.getRowCoordinate(loc))
                            + sign2 * (instance.getColCoordinate(n->location) - instance.getColCoordinate(loc));
                }
                if (0 <= barrier_id && barrier_id < num_barrier && !block[barrier_id] && barrier_time == n->level) {
                    if (n->children.size() == 1 && extent_L[barrier_id] == MAX_TIMESTEP &&
                        abs(dir1) * abs(n->location - n->children.front()->location) ==
                        instance.getCols());// the only child node is on the same barrier
                    else {
        ...
        for (int i = 0; i < num_barrier; i++) {
            if (block[i]) {
                ...
                if (abs(dir1) == 1) //vertical barriers
                {
                    ...
                    int time_offset =
                            timestep + i - sign1 * (instance.getColCoordinate(loc) - instance.getColCoordinate(start));
                    ...
                    int time_offset =
                            timestep + i - sign1 * (instance.getRowCoordinate(loc) - instance.getRowCoordinate(start));
    ...
    bool RectangleReasoning::isCut(const Constraint &b, const pair<int, int> &Rs, const pair<int, int> &Rg) {
        ...
        if (b_l == b_u) {
            if (((Rs.first <= b_l.first && b_u.first <= Rg.first) ||
                 (Rs.first >= b_l.first && b_u.first >= Rg.first)) &&
                ((Rs.second <= b_l.second && b_u.second <= Rg.second) ||
                 (Rs.second >= b_l.second && b_u.second >= Rg.second)))
                return true;
            else
                return false;
        ...
        if (Rs.first <= b_l.first && b_l.first <= b_u.first && b_u.first <= Rg.first && b_l.second == b_u.second)
            return true;
        else if (Rs.second <= b_l.second && b_l.second <= b_u.second && b_u.second <= Rg.second &&
                 b_l.first == b_u.first)
            return true;
...
//Identify rectangle conflicts for CR/R
    bool RectangleReasoning::isRectangleConflict(const pair<int, int> &s1, const pair<int, int> &s2,
                                                 const pair<int, int> &g1, const pair<int, int> &g2, int g1_t,
                                                 int g2_t) {
...
// Return 0 if it is a non-cardinal rectangle conflict
    int RectangleReasoning::classifyRectangleConflict(const pair<int, int> &s1, const pair<int, int> &s2,
        ...
        if ((s2.first - s1.first) * (s1.first - g1.first) < 0 &&
            (s2.second - s1.second) * (s1.second - g1.second) < 0) // s1 in the middle
            return 0;
        else if ((s1.first - s2.first) * (s2.first - g2.first) < 0 &&
                 (s1.second - s2.second) * (s2.second - g2.second) < 0) // s2 in the middle
            return 0;
...
//Compute rectangle corner Rs
    pair<int, int>
    RectangleReasoning::getRs(const pair<int, int> &s1, const pair<int, int> &s2, const pair<int, int> &g1) {
        int x, y;
...
//Compute rectangle corner Rg
    pair<int, int>
    RectangleReasoning::getRg(const pair<int, int> &s1, const pair<int, int> &g1, const pair<int, int> &g2) {
        int x, y;
        if (s1.first == g1.first)
            x = g1.first;
        else if (s1.first < g1.first)
...
//Compute start candidates for RM
    list<int> RectangleReasoning::getStartCandidates(const MAPFPath &path, const MDD &mdd, int timestep) {
        list<int> starts;
...
//Compute goal candidates for RM
    list<int> RectangleReasoning::getGoalCandidates(const MAPFPath &path, const MDD &mdd, int timestep) {
        list<int> goals;
        for (int t = (int) path.size() - 1;
             t >= timestep; t--) //Find end that is single and Manhattan-optimal to conflicting location
        {
            if (mdd.levels[t].size() == 1 &&
                mdd.levels[t].front()->location == path[t].location &&
    ...
    int RectangleReasoning::getStartCandidate(const MAPFPath &path, int dir1, int dir2, int timestep) {
        for (int t = timestep;
             t > 0; t--) //Find the earliest start that is single and Manhattan-optimal to conflicting location
        {
    ...
    int RectangleReasoning::getGoalCandidate(const MAPFPath &path, int dir1, int dir2, int timestep) {
        for (int t = timestep; t < (int) path.size() -
                                   1; t++) //Find the latest end that is single and Manhattan-optimal to conflicting location
        {
...
/*int getRectangleTime(const Conflict& conflict, const std::vector<std::vector<PathEntry>*>& paths, int num_col)
{
    ...
    bool RectangleReasoning::addModifiedBarrierConstraints(int a1, int a2,
                                                           ...
                                                           list<Constraint> &constraint1,
                                                           list<Constraint> &constraint2) {
        if ((s2.first - s1.first) * (s1.first - Rg.first) > 0 &&
            (s2.second - s1.second) * (s1.second - Rg.second) > 0) // s1 in the middle
        {
            ...
            if (!found) {
                ...
                if (!addModifiedHorizontalBarrierConstraint(a1, mdd1, Rg.first, Rs.second, Rg.second, Rg_t,
                                                            constraint1))
                    return false;
            ...
            if (!found) {
                ...
                if (!addModifiedVerticalBarrierConstraint(a1, mdd1, Rg.second, Rs.first, Rg.first, Rg_t, constraint1))
                    return false;
                if (!addModifiedHorizontalBarrierConstraint(a2, mdd2, Rg.first, Rs.second, Rg.second, Rg_t,
                                                            constraint2))
                    return false;
                return true;
            }
        } else if ((s1.first - s2.first) * (s2.first - Rg.first) > 0 &&
                   (s1.second - s2.second) * (s2.second - Rg.second) > 0) // s2 in the middle
        {
            int Rs_t = Rg_t - instance.getManhattanDistance(Rs, Rg);
            // try horizontal first
            int offset = Rs.first > Rg.first ? 1 : -1;
...
// t_min and t_min + abs(y_start - y_end) are their corresponding timesteps.
    bool RectangleReasoning::hasNodeOnBarrier(const MDD *mdd, int y_start, int y_end, int x, int t_min,
...
// add a vertical modified barrier constraint
    ...
    bool RectangleReasoning::blocked(const MAPFPath &path, const list<Constraint> &constraints) {
    ...
    bool RectangleReasoning::traverse(const MAPFPath &path, int loc, int t) {
        if (t >= (int) path.size())
