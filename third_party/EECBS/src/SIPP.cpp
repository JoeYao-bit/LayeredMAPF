...
// yz: get path from LLNode goal and previous goal
...
// Returns a path that minimizes the collisions with the paths in the path table, breaking ties by the length
    MAPFPath SIPP::findPath(const ConstraintTable &constraint_table) {
        reset();
        ...
        while (!focal_list.empty()) {
            ...
            // yz: get next neighbor and their intervals, candidate of expansion
            for (int next_location : instance.getNeighbors(curr->location)) // move to neighboring locations
            {
                for (auto &i : reservation_table.get_safe_intervals(
                    ...
                    // tie: get value in i in its order
    ...
    MAPFPath SIPP::findOptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
...
// lowerbound is an underestimation of the length of the path in order to speed up the search.
    pair<MAPFPath, int> SIPP::findSuboptimalPath(const HLNode &node, const ConstraintTable &initial_constraints,
...
// yz: get the time cost from start to end, under constraints, using DFS
