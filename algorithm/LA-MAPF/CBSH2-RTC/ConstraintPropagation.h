...
#ifndef LAYEREDMAPF_CONSTRAINTPROPAGATION_CBSH2_RTC_H
#define LAYEREDMAPF_CONSTRAINTPROPAGATION_CBSH2_RTC_H
...
namespace freeNav::LayeredMAPF::LA_MAPF::CBSH2_RTC {
    ...
    public:
        ConstraintPropagation(MDD<N, State> *mdd0, MDD<N, State> *mdd1) :
                mdd0(mdd0), mdd1(mdd1) {}
        ...
        virtual void init_mutex() {
            ...
            std::unordered_map<int, MDDNode *> loc2mddThisLvl;
            std::unordered_map<int, MDDNode *> loc2mddNextLvl = collectMDDlevel(mdd1, 0);
        ...
        virtual void fwd_mutex_prop() {
            std::vector<boost::unordered_set<edge_pair> > to_check(std::max(mdd0->levels.size(), mdd1->levels.size()));
        ...
        virtual bool mutexed(int level_0, int level_1) {
            ...
            MDD<N, State> *mdd_s = mdd0;
            MDD<N, State> *mdd_l = mdd1;
            if (level_0 > level_1) {
        ...
        virtual std::pair<std::vector<Constraint>, std::vector<Constraint>> generate_constraints(int level_0, int level_1) {
            MDD<N, State> *mdd_s = mdd0;
            MDD<N, State> *mdd_l = mdd1;
            bool reversed = false;
            if (level_0 > level_1) {
            ...
            if (!non_mutexed.empty()) {
                ...
                Constraint length_con(0, goal_ptr_i->location, -1, level_0, level_0+2, constraint_type::GLENGTH);
                // std::pair<int, int> length_con = {-1, level_0};

                std::vector<Constraint> cons_vec_1;
                for (auto &it:cons_set_1) {
                    // TODO:
                    cons_vec_1.push_back(Constraint(1, it.second, it.second, -1, it.first, constraint_type::VERTEX));
                }
...
#endif //LAYEREDMAPF_CONSTRAINTPROPAGATION_H
