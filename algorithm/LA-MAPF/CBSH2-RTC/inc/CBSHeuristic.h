...
namespace freeNav::LayeredMAPF::LA_MAPF::CBSH2_RTC {
    ...
    public:
        ...
        CBS_Li::ConstraintTable* ct = nullptr; // yz: introduce static constraint for laryerd MAPF
        ...
        bool computeInformedHeuristics(CBSNode &curr,
                                       double time_limit) { // this function is called when poping a CT node for the first time
            ...
            switch (type) {
                ...
                case heuristics_type::CG:
                    ...
                    if (curr.parent == nullptr ||  // root node of CBS tree
                        target_reasoning || corridor_reasoning == corridor_strategy::STC ||
                        corridor_reasoning == corridor_strategy::GC ||
                ...
                case heuristics_type::DG:
                    ...
                    if (curr.parent == nullptr || // root node of CBS tree
                        target_reasoning || corridor_reasoning == corridor_strategy::STC ||
                        corridor_reasoning == corridor_strategy::GC ||
                        corridor_reasoning == corridor_strategy::DC ||
        ...
        void computeQuickHeuristics(CBSNode &node) const { // this function is called when generating a CT node
    ...
    private:
        ...
        bool buildWeightedDependencyGraph(CBSNode &node, vector<int> &CG) {
        ...
        int solve2Agents(int a1, int a2, const CBSNode &node, bool cardinal) {
            ...
            CBS cbs(engines, constraints, ct, initial_paths, screen);
            ...
            if(rst < 0) {
                if (cbs.runtime > time_limit - runtime || cbs.num_HL_expanded > node_limit) { // time out or node out
                    rst = (int) cbs.min_f_val - root_g; // using lowerbound to approximate
                    std::cout << "(int) cbs.min_f_val - root_g = " << (int) cbs.min_f_val << " - " <<  root_g << std::endl;
                } else if (cbs.solution_cost < 0) // no solution
                    rst = MAX_COST;
                else {
                    rst = cbs.solution_cost - root_g;
                    std::cout << "cbs.solution_cost - root_g = " << cbs.solution_cost << " - " << root_g << std::endl;
                }
            }
            assert(rst >= 0);
            return rst;
        ...
        static bool SyncMDDs(const MDD &mdd, const MDD &other) {    // Match and prune MDD according to another MDD.
        ...
        int minimumVertexCover(const vector<int> &CG, int old_mvc, int cols, int num_of_CGedges) {
            assert(old_mvc >= 0);
            clock_t t = clock();
            int rst = 0;
            if (num_of_CGedges < 2)
        ...
        int
        DPForWMVC(vector<int> &x, int i, int sum, const vector<int> &CG, const vector<int> &range, int &best_so_far) {
            if (sum >= best_so_far)
                return MAX_COST;
            double runtime = (double) (clock() - start_time) / CLOCKS_PER_SEC;
            if (runtime > time_limit)
