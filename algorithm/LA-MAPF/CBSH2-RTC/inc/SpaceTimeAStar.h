...
namespace freeNav::LayeredMAPF::LA_MAPF::CBSH2_RTC {
    ...
    public:
        ...
        AStarNode(int loc, int g_val, int h_val, LLNode *parent, int timestep, int num_of_conflicts = 0,
        ...
        struct eqnode {
            bool operator()(const AStarNode *s1, const AStarNode *s2) const {
                return (s1 == s2) || (s1 && s2 &&
                                     ...
                                     s1->unsatisfied_positive_constraint_sets ==
                                     s2->unsatisfied_positive_constraint_sets;
            }
        };
    };
    ...
    public:
        ...
        Path findPath(const CBSNode &node, const ConstraintTable &initial_constraints,
            ...
            if(this->ct != nullptr) {
                if (this->ct->length_min > constraint_table.length_max ||  // the agent cannot reach its goal location
                    this->ct->constrained(start_location, 0)) // the agent cannot stay at its start location
                {
                    return Path();
                }
            }
            ...
            if (constraint_table.getNumOfLandmarks() > 0) {
                ...
                for (const auto constraint : node.constraints) {
                    ...
                    switch (type) {
                        ...
                        case constraint_type::POSITIVE_VERTEX:
                            if (agent != a) {
                                if ((paths[agent]->size() > t && paths[agent]->at(t).location == x) ||
                                    paths[agent]->back().location == x) {
                                    t_max = max(t_max, t);
                                    t_min = min(t_min, t);
                                }
                            }
                        ...
                        case constraint_type::POSITIVE_EDGE:
                            if (agent != a) {
                                curr = (paths[agent]->size() <= t) ? paths[agent]->back().location : paths[agent]->at(
                                        t).location;
                                prev = (paths[agent]->size() <= t - 1) ? paths[agent]->back().location : paths[agent]->at(
                                        t - 1).location;
                        ...
                        case constraint_type::BARRIER:
                            if (agent == a) {
                                ...
                                for (const auto &state : states) {
                                    curr = (paths[agent]->size() <= state.second) ? paths[agent]->back().location
                                                                                  : paths[agent]->at(state.second).location;
                                    if (curr == state.first) {
                        ...
                        case constraint_type::RANGE:
                            if (agent == a) {
                                for (int i = y; i <= t; i++) {
                                    curr = (paths[agent]->size() <= i) ? paths[agent]->back().location : paths[agent]->at(
                                            i).location;
                                    if (curr == x) {
                                        t_max = max(t_max, i);
                                        t_min = min(t_min, i);
                ...
                if (goal_state.second == MAX_TIMESTEP) {
                    ...
                    return path;
                } else if ((int) paths[agent]->size() <=
                           goal_state.second) // the landmark is at the goal location after the agent reaches
                {
        ...
        int getTravelTime(int end, const ConstraintTable &constraint_table, int upper_bound) {
            ...
            if (constraint_table.length_min >= MAX_TIMESTEP || constraint_table.length_min > constraint_table.length_max ||
                // the agent cannot reach its goal location
                constraint_table.constrained(start_location, 0)) // the agent cannot stay at its start location
            {
                return length;
            }
    ...
    private:
        ...
        Path findShortestPath(ConstraintTable &constraint_table, const pair<int, int> start_state, int lowerbound) {
            ...
            if(this->ct != nullptr) {
                holding_time = max(holding_time, ct->getHoldingTime(goal_location, 0));
                //holding_time = max(holding_time, ct->length_min);
                //holding_time = max(holding_time, constraint_table.length_min); // yz: avoid assert failed
            }
            lower_bound = max(holding_time - start_state.second, max(min_f_val, lowerbound));

            if(this->ct != nullptr) {
                //lower_bound = max(lower_bound, ct->getHoldingTime(goal_location, 0) + 1);
                //holding_time = max(lower_bound, holding_time); // yz: avoid assert rst < 0 failed
            }
            ...
            while (!open_list.empty()) {
                ...
                for (int next_location : next_locations) {
                    ...
                    if (max((int) constraint_table.cat_size, constraint_table.latest_timestep) + 1 <
                        curr->timestep) { // now everything is static, so switch to space A* where we always use the same timestep
                        if(ct != nullptr) {
                            //if (curr->timestep > holding_time)
                            { // yz: considering static constraint
                                if (next_timestep > this->ct->getMaxTimestep())
                                {
                    ...
                    if(this->ct != nullptr) {
                        ...
                        if(next_location == goal_location) {
                            if(next_timestep < ct->getHoldingTime(next_location, 0)) {
                                continue;
                            }
                        }
                    }
                    ...
                    keep = constraint_table.updateUnsatisfiedPositiveConstraintSet(
                            curr->unsatisfied_positive_constraint_sets, next->unsatisfied_positive_constraint_sets,
                            next_location, next_timestep);
                    if (!keep) {
        ...
        Path findPath(ConstraintTable &constraint_table, const pair<int, int> start_state, const pair<int, int> goal_state) {
            // generate start and add it to the OPEN & FOCAL list
