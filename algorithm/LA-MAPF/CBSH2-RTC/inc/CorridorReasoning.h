    ...
    private:
        ...
        shared_ptr<Conflict> findCorridorConflict(const shared_ptr<Conflict> &conflict,
            ...
            if (enter_time[0] > enter_time[1]) {
                int temp = enter_time[0];
                enter_time[0] = enter_time[1];
                enter_time[1] = temp;
                temp = a[0];
                a[0] = a[1];
                a[1] = temp;
        ...
        shared_ptr<Conflict> findCorridorTargetConflict(const shared_ptr<Conflict> &conflict,
            ...
            int a[2] = {conflict->a1, conflict->a2};
            int entry[2] = {-1, -1}, exit[2] = {-1, -1}, start[2] = {-1, -1}, goal[2] = {-1,
                                                                                         -1}; // store the corresponding node indices in the corridor
            int goal_time[2] = {-1, -1};
            for (int i = 0; i < 2; i++) {
            ...
            if ((max(start[0], entry[0]) - max(start[1], entry[1])) * (max(goal[0], exit[0]) - max(goal[1], exit[1])) >=
                0) // the start(entry) and goal(exit) locations need to be swaped
                return nullptr;
            ...
            { // the is a corridor-target conflict no matter the two agents move in the same direction or not
                ...
                if (start[1] == goal[1] && start[1] >=
                                           0)  // if the start and goal locations of one agent are the same (and, of course, inside the corridor).
                ...
                t1 = search_engines[a[middle_agent]]->getTravelTime(corridor.back(), ct1,
                                                                    l1 - corridor_length + goal[middle_agent]) - 1;
                t2 = search_engines[a[1 - middle_agent]]->getTravelTime(corridor.back(), ct2,
                                                                        l1 - corridor_length + goal[middle_agent]);
                l1 = min(l1, max(t1, t2) + corridor_length - goal[middle_agent]);
                if (l1 < (int) paths[a[middle_agent]]->size() -
                         1) // the length constraint for the left child node will not change the paths of the agents
                    return nullptr;
                ...
                int dir = max(goal[1 - middle_agent], exit[1 - middle_agent]) -
                          max(start[1 - middle_agent], entry[1 - middle_agent]);
                assert(dir !=
                       0); // the start and goal locations of the other agent cannot be the same, otherwise, the two agents didn't swap their relative locations
                dir = dir / abs(dir);
                ...
                {
                    if (l2 < (int) paths[a[1 - middle_agent]]->size() -
                             1) // the length constraint below will not change the path of the agent
                        return nullptr;
                    C2.emplace_back(a[1 - middle_agent], edge.first, -1, l2,
                                    constraint_type::GLENGTH); // add length constraint to C2
                } else {
                    if (goal_time[1 - middle_agent] >= l2) // the range constraint does not block the current path
                        return nullptr;
                    C2.emplace_back(a[1 - middle_agent], edge.first, 0, l2,
                                    constraint_type::RANGE); // add range constraint to C2
                }
                ...
                assert(dir !=
                       0); // the start and goal locations of the agent cannot be the same, otherwise, the two agents didn't swap their relative locations
                dir = dir / abs(dir);
                auto t = getTimeRanges(a[0], a[1], corridor[exit[0]], corridor[exit[1]],
                                       corridor[exit[0] - dir], corridor[exit[1] + dir], goal_time[0], goal_time[1],
                                       corridor_length, node);
        ...
        static int
        getCorridorLength(const std::vector<PathEntry> &path, int t_start, int loc_end, std::pair<int, int> &edge) {
        ...
        pair<int, int> getTimeRanges(int a1, int a2, int endpoint1, int endpoint2,
                                     int from1, int from2, int lowerbound1, int lowerbound2, int corridor_length,
                                     const CBSNode &node) {
