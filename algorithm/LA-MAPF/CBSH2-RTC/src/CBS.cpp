...
// also, do the same for ll_min_f_vals and paths_costs (since its already "on the way").
    ...
    void CBS::classifyConflicts(CBSNode &node) {
        ...
        while (!node.unknownConf.empty()) {
            ...
            if (rectangle_helper.strategy != rectangle_strategy::NR &&
                ...
                (int) paths[con->a2]->size() > timestep &&
                //conflict happens before both agents reach their goal locations
    ...
    bool CBS::findPathForSingleAgent(CBSNode *node, int ag, int lowerbound) {
        ...
        // find a path
        //lowerbound = max(lowerbound, (int) paths[ag]->size()); // yz: to avoid assert rst < 0 failed, but cause single test failed
...
// adding new nodes to FOCAL (those with min-f-val*f_weight between the old and new LB)
    ...
    void CBS::saveStats(const string &fileName, const string &instanceName) const {
        ...
        for (auto ins : heuristic_helper.sub_instances) {
            stats << get<0>(ins) << "," << get<1>(ins) << "," << get<2>(ins)->time_generated << "," << get<3>(ins)
                  << "," << get<4>(ins) << endl;
        }
        stats.close();
    ...
    bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound) {
        ...
        while (!open_list.empty() && !solution_found) {
            ...
            if (runtime > time_limit || num_HL_expanded > node_limit
                || heuristic_helper.sub_instances.size() >= MAX_NUM_STATS) {  // time/node out
                solution_cost = -1;
                solution_found = false;
                break;
            }
                ...
                if (disjoint_splitting && curr->conflict->type == conflict_type::STANDARD) {
                    ...
                    if (curr->conflict->type == conflict_type::RECTANGLE &&
                        ...
                        for (const auto constraint : child[1 - i]->constraints) {
                            child[i]->constraints.emplace_back(get<0>(constraint), get<1>(constraint),
                                                               get<2>(constraint), get<3>(constraint),
                        ...
                        child[i]->constraints.emplace_back(get<0>(constraint), get<1>(constraint), get<2>(constraint),
                                                           get<3>(constraint),
    ...
    CBS::CBS(vector<SingleAgentSolver *> &search_engines,
        ...
        heuristic_helper.ct = ct;
    }

    CBS::CBS(const Instance &instance, CBS_Li::ConstraintTable* ct, bool sipp, int screen) :
            screen(screen), focal_w(1),
            ...
            corridor_helper(search_engines, initial_constraints),
            heuristic_helper(instance.getDefaultNumberOfAgents(), paths, search_engines, initial_constraints,
                             mdd_helper) {
        heuristic_helper.ct = ct;
        this->ct = ct;
        clock_t t = clock();
    ...
    bool CBS::generateRoot() {
        ...
        if (paths_found_initially.empty()) {
            ...
            for (auto i : agents) {
                ...
                paths_found_initially[i] = search_engines[i]->findPath(*dummy_start, initial_constraints[i], paths, i,
                                                                       0);
