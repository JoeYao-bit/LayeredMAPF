#include "../inc/RectangleReasoning.h"
#include <memory>
#include <queue>
#include <stack>
...
// for R
    shared_ptr<Conflict>
    RectangleReasoning::findRectangleConflict(const vector<Path *> &paths, int timestep, int a1, int a2) {
...
// for GR
    shared_ptr<Conflict>
    RectangleReasoning::findGenerealizedRectangleConflict(const vector<Path *> &paths, int timestep,
                                                          int a1, int a2, const MDD *mdd1, const MDD *mdd2) {
        ...
        pair<int, int> Rs(paths[a1]->at(timestep - 1).location, conflict_location), Rg(conflict_location,
                                                                                       conflict_location);
        findOverlapArea(conflict_location, overlap_area, visit_times1, visit_times2, entry1, entry2, exit1, exit2, Rs,
                        Rg, &multiple_visits);
...
//Identify rectangle conflicts for RM
    bool RectangleReasoning::isRectangleConflict(const pair<int, int> &s1, const pair<int, int> &s2,
        ...
        return !(s1 == s2 || // A standard cardinal conflict
                 ...
                 (s1.first == g1.first && s2.second == g2.second) ||
                 (s1.second == g1.second && s2.first == g2.first)); // area = 1 <==> a cardinal vertex conflct
    }
...
//////////////////////////////////////////////////////////////////////////////
    ...
    void RectangleReasoning::findOverlapArea(int conflict_location, vector<bool> &overlap_area,
                                             const vector<MDDNode *> &visit_times1,
                                             const vector<MDDNode *> &visit_times2,
                                             set<pair<int, int> > &entry1, set<pair<int, int> > &entry2,
                                             list<int> &exit1, list<int> &exit2,
                                             pair<int, int> &Rs, pair<int, int> &Rg,
                                             const MDDNode *multiple_visits) const {
    ...
    list<Constraint> RectangleReasoning::generateConstraint(const vector<bool> &overlap_area,
                                                            const vector<MDDNode *> &visit_times, int agent,
                                                            const pair<int, int> &from,
                                                            const pair<int, int> &to) const // generate constraints from from to to along the perimeter counter-clockwisely
    {
        list<Constraint> constraints;
    ...
    void RectangleReasoning::printOverlapArea(const vector<Path *> &paths, int conflict_timestep,
        ...
        list<int> exit1, exit2; // exit locations
        vector<bool> overlap_area(instance.map_size, false);
        pair<int, int> Rs(paths[a1]->at(conflict_timestep - 1).location, conflict_location), Rg(conflict_location,
                                                                                                conflict_location);
        findOverlapArea(conflict_location, overlap_area, visit_times1, visit_times2, entry1, entry2, exit1, exit2, Rs,
                        Rg, &multiple_visits);
        cout << "Overlap area: ";
