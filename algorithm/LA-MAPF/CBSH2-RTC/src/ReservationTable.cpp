#include "../inc/ReservationTable.h"
        ...
        for (size_t ag = 0; ag < paths.size(); ag++) {
            ...
            for (size_t timestep = 0; timestep < paths[ag]->size(); timestep++) {
                ...
                if (prev_location != curr_location) {
                    ...
                    cat[getEdgeIndex(curr_location, prev_location)].emplace_back(timestep,
...
// [lower_bound, upper_bound)
    list<Interval>
    ReservationTable::get_safe_intervals(size_t from, size_t to, size_t lower_bound, size_t upper_bound) {
