...
namespace freeNav::LayeredMAPF::LA_MAPF::CBSH2_RTC {
    ...
    public:
        ...
        MutexReasoning(const Instance &instance, const std::vector<ConstraintTable<N, State> > &initial_constraints) :
    ...
    private:
        ...
        bool constraint_applicable(const std::vector<Path *> &paths, std::list<Constraint> &constraint) {
        ...
        findMutexConflict(const std::vector<Path *> &paths, int a1, int a2, CBSNode &node, MDD<N, State> *mdd_1, MDD<N, State> *mdd_2) {
            ...
            for (auto con:a) {
                get<0>(con) = a1;
                mutex_conflict->cs1.push_back(con);
            }

            for (auto con:b) {
                get<0>(con) = a2;
                mutex_conflict->cs2.push_back(con);
            }
