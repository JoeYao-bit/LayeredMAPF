//
// Created by yaozhuo on 1/5/26.
//

#ifndef LAYEREDMAPF_MDD_CBSH2_RTC_H
#define LAYEREDMAPF_MDD_CBSH2_RTC_H
...
namespace freeNav::LayeredMAPF::LA_MAPF::CBSH2_RTC {
    ...
    class MDD {
    private:
        std::shared_ptr<SingleAgentSolver<N, State> > solver;
    ...
    public:
        ...
        bool buildMDD(const ConstraintTable<N, State> &ct,
            ...
            for (auto parent : goal_node->parents) {
                if (parent->location ==
                    goal_node->location) // the parent of the goal node should not be at the goal location
                {
                    del = parent;
                    continue;
                }
        ...
        void increaseBy(const ConstraintTable<N, State> &ct, int dLevel, std::shared_ptr<SingleAgentSolver<N, State> > solver) {
        ...
        MDD(const MDD &cpy) {
            ...
            for (size_t t = 0; t < levels.size() - 1; t++) {
                for (auto node = levels[t].begin(); node != levels[t].end(); ++node) {
                    ...
                    for (std::list<MDDNode *>::const_iterator cpyChild = cpyNode->children.begin();
                         cpyChild != cpyNode->children.end(); ++cpyChild) {
    ...
    template<Dimension N, typename State>
    std::ostream &operator<<(std::ostream &os, const MDD<N, State> &mdd) {
        for (const auto &level : mdd.levels) {
            std::cout << "L" << level.front()->level << ": ";
    ...
    public:
        ...
        MDD<N, State> *getMDD(CBSNode &node, int id, size_t mdd_levels) {
    ...
    template <Dimension N, typename State>
    std::unordered_map<int, MDDNode *> collectMDDlevel(MDD<N, State> *mdd, int i)  {
        std::unordered_map<int, MDDNode *> loc2mdd;
